...
// Define strict Prisma payload types
...
type BookingWithDetails = Prisma.BookingRequestGetPayload<{
  include: { student: true };
}> & { hostelName: string; hostelPrice: number };
...
export default async function ManagerDashboard() {
  ...
  const allBookings: BookingWithDetails[] = hostels.flatMap((h: HostelWithBookings) =>
    h.bookings.map((b) => ({ ...b, hostelName: h.name, hostelPrice: h.monthlyPrice }))
  );
  ...
  const totalCapacity = hostels.reduce((acc: number, h: HostelWithBookings) => {
    const occupiedInHostel = h.bookings.filter((b) => b.status === "CONFIRMED").length;
  ...
  // Gamification: Calculate locked revenue in pending requests
  const lockedRevenue = pendingBookings.reduce((acc, b) => acc + b.hostelPrice, 0);
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <h1 className="text-4xl md:text-5xl font-black text-white tracking-tight mb-2">
              Hello, {session.user.name?.split(" ")[0] || "Manager"} <span className="animate-wave inline-block origin-[70%_70%]">👋</span>
            </h1>
            <p className="text-sm md:text-base text-cyan-100/70 max-w-xl leading-relaxed">
              Command central for your properties. Monitor occupancy, manage students, and scale your hostel empire.
            </p>
          </div>
          
          {/* Quick Stat inside Header */}
        ...
        <div className="lg:col-span-8 space-y-10">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between pb-2">
              ...
              {pendingBookings.length > 0 && (
                <span className="flex items-center gap-1.5 text-xs bg-rose-100 text-rose-700 font-bold px-3 py-1.5 rounded-full animate-pulse border border-rose-200">
            ...
            {pendingBookings.length === 0 ? (
              <div className="bg-white py-16 px-6 rounded-[2rem] text-center flex flex-col items-center justify-center shadow-sm">
                <div className="w-20 h-20 bg-[#ecfeff] rounded-full flex items-center justify-center mb-4">
                  <svg className="w-10 h-10 text-cyan-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M5 13l4 4L19 7"></path></svg>
                </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 text-xs font-medium text-slate-500">
                        ...
                        <span className="flex items-center gap-1.5 text-slate-700">
                          <svg className="w-4 h-4 text-cyan-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path></svg>
                          {booking.hostelName}
                        </span>
                      </div>
            ...
            {hostels.length === 0 ? (
              <div className="bg-white p-12 rounded-[2rem] text-center flex flex-col items-center justify-center shadow-sm">
                ...
                <p className="text-sm text-slate-500 max-w-sm mt-2 leading-relaxed">
                  You haven't listed any properties yet. Use the form to publish your first hostel and attract student bookings.
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {hostels.map((hostel: HostelWithBookings) => {
                  const occupied = hostel.bookings.filter(b => b.status === "CONFIRMED").length;
                    ...
                    >
                      ...
                      <div>
                        <div className="flex justify-between items-start gap-2 mb-4">
                          ...
                          {isHighDemand && (
                            <span className="text-[10px] font-black uppercase tracking-widest text-orange-600 bg-orange-100 px-3 py-1.5 rounded-lg flex items-center gap-1">
                              🔥 High Demand
                            </span>
                          )}
                          {isFull && (
                            <span className="text-[10px] font-black uppercase tracking-widest text-rose-600 bg-rose-100 px-3 py-1.5 rounded-lg">
                              Fully Booked
                            </span>
                          )}
                        </div>
